...
/**
 * startTokens() - Initialize a token stream of tokens.
 * @maxLength - the maximum token length
 *
...
/**
 * getToken() - Return the next token in the token stream.
 *
 ...
 * store the value in token until the next call to getToken(), and no longer.
 *
 ...
 * copy of the return value from getToken():
 *
 *     strcpy(token, getToken());
 *
 * rather than the above assignment. In this syntax the string copy acts much
 * more like a true assignment, and using getToken() you can store the next
 ...
 * startTokens()). This function is not guaranteed to work for longer tokens,
